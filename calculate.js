...
const standardForm = (iobj, irows) => {
    ...
    printTableCardStandardForm('Matriz de coeficientes tras sumar variables de holgura, excedentes y artificiales:')
    printVariables('no básicas')
...
const simplex = (phase) => {
    ...
    printRatio(card)
    if ($.leavingIndex === -1) {
        const msg = 'Todas las razones mínimas son negativas o infinitas, por lo que la solución no tiene límites.'
    ...
    if (!historyNotRepeat) {
        //const msg = 'La degeneración existe, deteniéndose.'
        //printWarning(msg, card)
        return false
    }
...
const removeArtificial = () => {
    ...
    printWarning('Se eliminan las variables artificiales.', output)
}

const phase1 = () => {
    printSubtitle('Fase 1: eliminación de variables artificiales')
    $.dim = getDim()
...
const phase2 = () => {
    printSubtitle('Fase 2: Encontrar la solución óptima')
    $.dim = getDim()
    while ($.kount <= $.maxIter) {
        $.basicVars = getBasicVars()
        if (!simplex(2)) break
...
const getProblem = () => {
    ...
    if (input !== '') {
        ...
        printWarning('No hay valores de entrada', emptyMsg)
    }
}
