const defaultInput = 'max = 30x1 + 50x2\nx1 + 3x2 <= 200\nx1 + x2 <= 100\nx1 >= 20\nx2 >= 10'
...
const printVariables = (q) => {
    const vars = q === 'básicas' ? $.variables : $.variables.slice($.basicKount)
    const txt = vars.join(', ')
    const div = createNode('div', ['block'], `Variables ${q}: ${txt}`)
...
const printEnteringLeavingVar = (card) => {
    ...
    const word = $.target === 'min' ? 'tiene el más bajo' : 'tiene el más alto'
    const ev1 = `Variable de entrada: Entre todos los costes relativos`
    ...
    const lv1 = `Variable saliente: Entre todas las razones`
    const lv2 = `${checkDecimals($.ratio[$.leavingIndex])} tiene el más bajo`
